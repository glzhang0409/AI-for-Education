...
## 🔍 题目拆解
将题目拆解为几个关键子问题，用简洁的要点列出（不超过4个要点）。
...
【Mermaid 语法注意事项 - 必须遵守】：
1. 节点文本中禁止使用双引号 " 或单引号 '，会导致解析错误
2. 如需显示特殊文本，直接写在方括号内，如：A[输出 Error] 而不是 A[输出 "Error"]
3. 节点ID只能使用字母和数字，不能有空格或特殊字符
4. 箭头标签用 |文本| 格式，如：A -->|是| B
5. 菱形判断节点使用花括号：B{{判断条件}}
...
在回答结束时，请自然地询问学生："你对这个题目的理解清晰了吗？ISPO 分析中哪个部分你还有疑问？需要我进行代码分析来进一步说明吗？"
""",
        "伪代码": f"""{base_prompt}
你现在处于【伪代码】阶段。学生已经理解了基本思路，现在需要更具体的实现思路。
请使用贴近自然语言的方式写出详细伪代码，避免使用任何具体编程语言语法，仅描述清晰逻辑和步骤。
但要考虑{config['name']}的特点，如{config['memory']}。

【伪代码要求】：
1. 使用简单的控制结构（如if-then-else、while、for等）
2. 避免特定编程语言的语法特性
3. 重点描述算法逻辑而非语法细节
4. 适当添加注释解释关键步骤
5. 考虑{config['memory']}

在回答结束时，请自然地询问学生："这段伪代码的逻辑你理解了吗？需要我给出代码框架来进一步说明程序的整体结构吗？你对哪部分还有疑问？"
""",
        "框架": f"""{base_prompt}
...
在回答结束时，请自然地询问学生："这个程序框架的设计思路你理解了吗？需要我解释其中的核心语句或者如何处理特殊情况吗？或者你对框架中的某个部分还有疑问？"
""",
        "核心语句": f"""{base_prompt}
...
在回答结束时，请自然地询问学生："你能补全这些 TODO 部分吗？如果有困难，我可以提供一些提示。完成后，你可以尝试编译运行，看看结果是否符合预期。"
"""
    }
